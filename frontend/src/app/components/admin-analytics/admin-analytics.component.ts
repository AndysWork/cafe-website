...
import { AdminAnalyticsCalculationService } from '../../services/admin-analytics-calculation.service';
import {
  getIstNow,
  getIstDateString,
...
export class AdminAnalyticsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private calcService: AdminAnalyticsCalculationService
  ) {}
  ...
  filterByDateRange(sales: Sales[]): Sales[] {
    return this.calcService.filterByDateRange(sales, this.dateRange) as Sales[];
  }
  ...
  calculateInsights() {
    ...
    const weeklyTrend = this.calcService.calculateWeeklyTrend(this.salesData);
    const monthlyComparison = this.calcService.calculateMonthlyComparison(this.salesData);
    const peakSalesDays = this.calcService.calculatePeakDays(this.salesData);
    const growthRate = this.calcService.calculateGrowthRate(this.salesData, this.allSalesData, this.dateRange);
    const itemCategoryBreakdown = this.calcService.calculateCategoryBreakdown(
      this.salesData, (name) => this.calcService.categorizeItem(name)
    );
  ...
  calculateWeeklyTrend(): { week: string; total: number }[] {
    return this.calcService.calculateWeeklyTrend(this.salesData);
  }
  ...
  calculateMonthlyComparison(): {
    ...
    return this.calcService.calculateMonthlyComparison(this.salesData);
  }

  calculatePeakDays(): { day: string; total: number }[] {
    return this.calcService.calculatePeakDays(this.salesData);
  }

  calculateGrowthRate(): number {
    return this.calcService.calculateGrowthRate(this.salesData, this.allSalesData, this.dateRange);
  }

  calculateCategoryBreakdown(): { category: string; count: number; revenue: number }[] {
    return this.calcService.calculateCategoryBreakdown(this.salesData, (name) => this.calcService.categorizeItem(name));
  }

  categorizeItem(itemName: string): string {
    return this.calcService.categorizeItem(itemName);
  }
  ...
  calculatePlatformMetrics(sales: any[], platform: string, platformCharges: any[]) {
    return this.calcService.calculatePlatformMetrics(sales, platform, platformCharges, this.dateRange);
  }
  ...
  formatCurrency(amount: number): string {
    return this.calcService.formatCurrency(amount);
  }
  ...
  formatPercentage(value: number): string {
    return this.calcService.formatPercentage(value);
  }
  ...
  getCategoryIcon(category: string): string {
    return this.calcService.getCategoryIcon(category);
  }

  getExpenseTypeIcon(type: string): string {
    return this.calcService.getExpenseTypeIcon(type);
  }
  ...
  calculateProportionalMonthlyExpense(
    ...
    return this.calcService.calculateProportionalMonthlyExpense(month, year, monthlyAmount, rangeStart, rangeEnd);
  }
