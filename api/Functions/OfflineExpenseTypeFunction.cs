...
public class OfflineExpenseTypeFunction
{
    ...
    public OfflineExpenseTypeFunction(IFinanceRepository mongo, AuthService authService, ILoggerFactory loggerFactory)
    {
        _mongo = mongo;
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "offlineexpensetypes")] HttpRequestData req)
    {
        try
        {
            ...
            var (request, validationError) = await ValidationHelper.ValidateBody<CreateOfflineExpenseTypeRequest>(req);
